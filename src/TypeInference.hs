...
substApplyExpr s = bimap (Subst.applyTySchema s) (Subst.applyTy s)
...
infer c e = do
  case runNameGenTWithout (Context.extractTypeVars c) (impl c e) of
    Left msg -> Left msg
    Right (s, cs, expr) -> Right (s, cs, expr)
  where cat = Subst.comp
        ...
        checkBinOpElements con t1 t2 t c e1 e2 = do
          ...
          let cs = Set.union (app s4 (app s3 (app s2 cs1))) (app s4 (app s3 cs2))
          let appE = substApplyExpr
          let e1'' = appE s4 (appE s3 (appE s2 e1'))
          let e2'' = appE s4 (appE s3 e2')
        ...
        impl c (AbsRec _ f x e1) = do
          ...
          let cs = Set.insert (beta', pi) (app s1 cs1)
          return (s2 `o` s1, cs, AbsRec ty f x (substApplyExpr s2 e1'))
        impl c (App _ e1 e2) = do
          ...
          let e1'' = substApplyExpr s3 (substApplyExpr s2 e1')
          let e2'' = substApplyExpr s3 e2'
        ...
        impl c (If _ e0 e1 e2) = do
          ...
          s4 <- lift $ unify (appTy s3 e2Ty) (appTy s3 (appTy s2 e1Ty))
          ...
          let appE = substApplyExpr
          let e0'' = appE s4 (appE s3 (appE s2 (appE s1 e0')))
          let e1'' = appE s4 (appE s3 (appE s2 e1'))
          let e2'' = appE s4 (appE s3 e2')
        ...
        impl c (Let _ (x, _) e1 e2) = do
          ...
          let e1'' = substApplyExpr s2 e1'
          let e1Ty'' = Subst.applyTySchema s2 e1Ty'
